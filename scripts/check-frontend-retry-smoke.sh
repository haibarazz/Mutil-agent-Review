...
SMOKE_SCREENSHOT="${SMOKE_SCREENSHOT:-/tmp/paper-review-frontend-retry-smoke.png}"
SMOKE_TMP_DIR="${SMOKE_TMP_DIR:-$(mktemp -d /tmp/paper-review-retry-smoke.XXXXXX)}"
...
if [[ -z "${CHROME}" ]]; then
  echo "Chrome is required for frontend retry smoke. Set CHROME_BIN to a Chrome-compatible binary." >&2
  exit 1
fi
...
if url_ok "${BACKEND_URL}/health"; then
  echo "Backend port ${BACKEND_PORT} is already in use; choose a free BACKEND_PORT for retry smoke." >&2
  exit 1
fi

if url_ok "${FRONTEND_URL}"; then
  echo "Frontend port ${FRONTEND_PORT} is already in use; choose a free FRONTEND_PORT for retry smoke." >&2
  exit 1
fi
...
paper = data_dir / "uploads" / "codex_retry_smoke.md"
paper.parent.mkdir(parents=True, exist_ok=True)
paper.write_text(
    "Retry Smoke Paper\n\nAbstract\nA canceled job for frontend retry smoke.\n\n1 Introduction\nContent.",
    encoding="utf-8",
)
...
LLM_PROVIDER=mock \
PORT="${BACKEND_PORT}" \
  "${PYTHON_BIN}" -m uvicorn src.api.app:app --host 127.0.0.1 --port "${BACKEND_PORT}" >/tmp/paper-review-retry-smoke-backend.log 2>&1 &
BACKEND_PID="$!"
...
) >/tmp/paper-review-retry-smoke-frontend.log 2>&1 &
FRONTEND_PID="$!"
...
"${CHROME}" \
  ...
  --user-data-dir="/tmp/paper-review-retry-smoke-chrome-${CDP_PORT}" \
  about:blank >/tmp/paper-review-retry-smoke-chrome.log 2>&1 &
CHROME_PID="$!"
...
echo "Frontend retry smoke screenshot: ${SMOKE_SCREENSHOT}"
echo "Frontend retry smoke data dir: ${SMOKE_DATA_DIR}"
