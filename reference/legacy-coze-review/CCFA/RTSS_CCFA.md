...
- 简写：RTSS
- 全名：IEEE Real-Time Systems Symposium
- 文件名：RTSS_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
RTSS 是 CCF 推荐目录中 **交叉/综合/新兴** 领域的 A 类会议。交叉、综合和新兴方向的顶级 venue，适合 Web、实时系统、计算生物、跨学科计算和新兴计算问题研究。
...
- 论文贡献与 IEEE Real-Time Systems Symposium 的全名和领域传统一致。
...
- RTSS 2026: http://2026.rtss.org/

来源类型：官方 CFP / 官方会议页面

### 官方定位与主题范围

- RTSS 官方定位为 real-time systems 顶级会议。
- 适合实时调度、实时操作系统、cyber-physical systems、嵌入式系统、实时网络、可预测性、mixed-criticality、实时 AI/edge。

### 官方投稿与评审要求

- 论文应围绕时间约束、可预测性、deadline、调度或实时系统可靠性提出理论、系统或实验贡献。
- 评审关注 temporal correctness、schedulability、worst-case analysis、系统实现和真实实时场景。

### 给智能体的硬约束

- RTSS fit 要求 real-time constraints 是核心。
- 普通嵌入式/边缘系统如果没有实时性问题，fit 弱。

## 智能体提示词片段

```text
目标 venue 是 IEEE Real-Time Systems Symposium (RTSS)，CCF-A，类型为会议，所属 CCF 专业领域为：交叉/综合/新兴 / Cross-disciplinary / Comprehensive / Emerging。
