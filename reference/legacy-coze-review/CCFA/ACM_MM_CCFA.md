...
- 简写：ACM MM
- 全名：ACM International Conference on Multimedia
- 文件名：ACM_MM_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
ACM MM 是 CCF 推荐目录中 **计算机图形学与多媒体** 领域的 A 类会议。计算机图形学与多媒体方向的顶级 venue，适合图形、视觉化、多媒体、VR/AR、渲染和交互媒体研究。
...
- 论文贡献与 ACM International Conference on Multimedia 的全名和领域传统一致。
...
- ACM MM 官方定位为 multimedia 旗舰会议。
- 适合 multimedia content analysis、retrieval、generation、multimodal learning、audio/video/image/text、systems、applications、HCI、social multimedia、responsible multimedia。

### 官方投稿与评审要求

- 论文应围绕多媒体数据和多模态问题提供方法、系统、数据集、应用或理论贡献，并充分评估。
- 评审关注多媒体任务相关性、跨模态贡献、实验严谨性和应用/系统影响。

### 给智能体的硬约束

- ACM MM fit 要求 multimedia/multimodal 核心。
- 普通 CV/NLP/ML 论文如果没有多媒体或跨模态贡献，fit 弱。

## 智能体提示词片段

```text
目标 venue 是 ACM International Conference on Multimedia (ACM MM)，CCF-A，类型为会议，所属 CCF 专业领域为：计算机图形学与多媒体 / Computer Graphics and Multimedia。
...
- 如果审稿人认为这是相邻领域论文，你如何解释它适合 ACM MM？
