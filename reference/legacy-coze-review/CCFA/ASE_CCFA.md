...
- 简写：ASE
- 全名：International Conference on Automated Software Engineering
- 文件名：ASE_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
ASE 是 CCF 推荐目录中 **软件工程/系统软件/程序设计语言** 领域的 A 类会议。软件工程、系统软件和程序设计语言方向的顶级 venue，适合软件开发、程序分析、编译器、操作系统、PL 和形式化方法研究。
...
- 论文贡献与 International Conference on Automated Software Engineering 的全名和领域传统一致。
...
- ASE 官方定位为 automated software engineering，关注自动化支持软件开发、维护、测试和演化。
- 适合 program analysis、testing、repair、synthesis、requirements、SE for AI、AI for SE、tools、developer productivity、empirical automated SE。

### 官方投稿与评审要求

- 论文应展示自动化方法对软件工程任务的实质帮助，包含工具、实验、数据集、用户/开发者研究或工业验证。
- 评审关注自动化程度、SE 问题重要性、实验严谨性和实践可用性。

### 给智能体的硬约束

- ASE fit 要求 software engineering automation 是核心。
- 只是用 LLM 写代码 demo，没有严谨 SE 任务和评估时 fit 弱。

## 智能体提示词片段

```text
目标 venue 是 International Conference on Automated Software Engineering (ASE)，CCF-A，类型为会议，所属 CCF 专业领域为：软件工程/系统软件/程序设计语言 / Software Engineering / System Software / Programming Languages。
