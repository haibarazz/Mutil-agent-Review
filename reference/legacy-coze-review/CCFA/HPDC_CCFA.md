# The International ACM Symposium on High-Performance Parallel and Distributed Computing (HPDC)
...
- 简写：HPDC
- 全名：The International ACM Symposium on High-Performance Parallel and Distributed Computing
- 文件名：HPDC_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
HPDC 是 CCF 推荐目录中 **计算机体系结构/并行与分布计算/存储系统** 领域的 A 类会议。计算机体系结构、并行与分布计算、存储系统方向的顶级 venue，适合硬件/系统/存储/HPC/云基础设施等底层计算系统研究。
...
- 论文贡献与 The International ACM Symposium on High-Performance Parallel and Distributed Computing 的全名和领域传统一致。
...
- HPDC 官方定位为 high-performance parallel and distributed computing，关注高性能并行/分布式计算系统。
- 适合 HPC、cloud/edge、并行运行时、调度、工作流、数据密集型计算、科学计算、加速器集群、分布式资源管理。

### 官方投稿与评审要求

- 论文应给出高性能/分布式计算问题、系统设计或算法，以及可扩展实验、真实应用或生产/科学 workload。
- 评审会关注 scalability、efficiency、robustness、资源利用、系统实现和跨平台表现。

### 给智能体的硬约束

- HPDC fit 不等于普通分布式应用；必须服务于 parallel/distributed high-performance computing。
- 缺少规模实验或高性能计算语境时 fit 弱。

## 智能体提示词片段

```text
目标 venue 是 The International ACM Symposium on High-Performance Parallel and Distributed Computing (HPDC)，CCF-A，类型为会议，所属 CCF 专业领域为：计算机体系结构/并行与分布计算/存储系统 / Computer Architecture / Parallel and Distributed Computing / Storage System。
