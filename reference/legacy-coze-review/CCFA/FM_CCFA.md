...
- 简写：FM
- 全名：International Symposium on Formal Methods
- 文件名：FM_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
FM 是 CCF 推荐目录中 **软件工程/系统软件/程序设计语言** 领域的 A 类会议。软件工程、系统软件和程序设计语言方向的顶级 venue，适合软件开发、程序分析、编译器、操作系统、PL 和形式化方法研究。
...
- 论文贡献与 International Symposium on Formal Methods 的全名和领域传统一致。
...
- FM 官方定位为 formal methods，关注形式化建模、规范、验证和证明在系统/软件/硬件中的应用。
- 适合 model checking、theorem proving、specification、refinement、runtime verification、formal analysis of software/hardware/cyber-physical/security systems。

### 官方投稿与评审要求

- 论文应有形式化模型、性质、证明/验证方法或实际应用案例，并说明 soundness、scalability 和工程意义。
- 评审关注形式化严谨性、工具支持和对真实系统的适用性。

### 给智能体的硬约束

- FM fit 必须有 formal specification/verification/reasoning 核心。
- 仅做测试或经验评估而无形式化方法时 fit 弱。

## 智能体提示词片段

```text
目标 venue 是 International Symposium on Formal Methods (FM)，CCF-A，类型为会议，所属 CCF 专业领域为：软件工程/系统软件/程序设计语言 / Software Engineering / System Software / Programming Languages。
