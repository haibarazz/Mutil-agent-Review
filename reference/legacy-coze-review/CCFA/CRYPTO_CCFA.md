...
- 简写：CRYPTO
- 全名：International Cryptology Conference
- 文件名：CRYPTO_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
CRYPTO 是 CCF 推荐目录中 **网络与信息安全** 领域的 A 类会议。网络与信息安全方向的顶级 venue，适合密码学、系统安全、网络安全、隐私、安全攻防和安全协议研究。
...
- 论文贡献与 International Cryptology Conference 的全名和领域传统一致。
...
- CRYPTO 官方定位为 cryptology/cryptography 理论与应用顶级会议。
- 适合密码学基础、协议、安全证明、零知识、多方计算、格/后量子、同态加密、签名、密码分析等。

### 官方投稿与评审要求

- 论文应有严格定义、定理、安全模型、证明或密码分析，通常要求形式化贡献。
- 实现和系统贡献只有在密码学新意或安全分析足够强时才适合。

### 给智能体的硬约束

- CRYPTO fit 必须有密码学核心贡献和形式化严谨性。
- 纯系统安全或应用部署没有密码学定理/模型时 fit 弱。

## 智能体提示词片段

```text
目标 venue 是 International Cryptology Conference (CRYPTO)，CCF-A，类型为会议，所属 CCF 专业领域为：网络与信息安全 / Network and Information Security。
...
- 如果审稿人认为这是相邻领域论文，你如何解释它适合 CRYPTO？
