...
- 简写：TON
- 全名：IEEE/ACM Transactions on Networking
- 文件名：TON_CCFA.md
- 类型：期刊
- 榜单归属：CCF-A
...
TON 是 CCF 推荐目录中 **计算机网络** 领域的 A 类期刊。计算机网络方向的顶级 venue，适合网络协议、网络系统、移动/无线网络、互联网测量和网络体系结构研究。
...
- 研究问题属于 **计算机网络** 的核心问题。
- 论文贡献与 IEEE/ACM Transactions on Networking 的全名和领域传统一致。
...
- IEEE/ACM Transactions on Networking About: https://dl.acm.org/journal/ton/about

来源类型：官方 Aims & Scope / About / Author 页面

### 官方定位与主题范围

- TON 官方定位为 networking 旗舰期刊，覆盖网络协议、架构、性能、理论、测量和系统。
- 适合 Internet、wireless、data center networks、routing、congestion control、network measurement、security、SDN/NFV、network algorithms。

### 官方投稿与评审要求

- 论文应提出网络领域原创结果，通常需要严谨理论、测量、仿真或系统实验。
- 期刊稿件需要完整性强，能解释模型假设、部署意义和与已有网络工作的关系。

### 给智能体的硬约束

- TON fit 必须是 networking contribution。
- 只有应用层系统或 ML 方法，没有网络机制/性能/协议贡献时 fit 弱。

## 智能体提示词片段

```text
目标 venue 是 IEEE/ACM Transactions on Networking (TON)，CCF-A，类型为期刊，所属 CCF 专业领域为：计算机网络 / Computer Networks。
