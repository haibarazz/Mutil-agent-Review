...
- 简写：TOS
- 全名：ACM Transactions on Storage
- 文件名：TOS_CCFA.md
- 类型：期刊
- 榜单归属：CCF-A
...
TOS 是 CCF 推荐目录中 **计算机体系结构/并行与分布计算/存储系统** 领域的 A 类期刊。计算机体系结构、并行与分布计算、存储系统方向的顶级 venue，适合硬件/系统/存储/HPC/云基础设施等底层计算系统研究。
...
- 论文贡献与 ACM Transactions on Storage 的全名和领域传统一致。
...
- ACM Transactions on Storage About: https://dl.acm.org/journal/tos/about

来源类型：官方 Aims & Scope / About / Author 页面

### 官方定位与主题范围

- TOS 官方定位为 storage 领域期刊，覆盖存储系统、文件系统、持久化、数据可靠性和存储技术。
- 适合 file systems、distributed storage、NVM/SSD、data protection、deduplication、storage security、performance modeling、cloud/HPC storage。

### 官方投稿与评审要求

- 论文应有存储问题定义、机制设计、可靠性/一致性语义和充分 workload/trace 评估。
- 期刊稿件需要清楚区分性能优化、可靠性保证和实现复杂度。

### 给智能体的硬约束

- TOS fit 必须以 storage 为核心，而非把存储作为背景设施。
- 建议智能体强制追问数据持久性、一致性、故障模型和 workload。

## 智能体提示词片段

```text
目标 venue 是 ACM Transactions on Storage (TOS)，CCF-A，类型为期刊，所属 CCF 专业领域为：计算机体系结构/并行与分布计算/存储系统 / Computer Architecture / Parallel and Distributed Computing / Storage System。
