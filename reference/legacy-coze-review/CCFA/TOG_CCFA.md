...
- 简写：TOG
- 全名：ACM Transactions on Graphics
- 文件名：TOG_CCFA.md
- 类型：期刊
- 榜单归属：CCF-A
...
TOG 是 CCF 推荐目录中 **计算机图形学与多媒体** 领域的 A 类期刊。计算机图形学与多媒体方向的顶级 venue，适合图形、视觉化、多媒体、VR/AR、渲染和交互媒体研究。
...
- 论文贡献与 ACM Transactions on Graphics 的全名和领域传统一致。
...
- ACM Transactions on Graphics About: https://dl.acm.org/journal/tog/about

来源类型：官方 Aims & Scope / About / Author 页面

### 官方定位与主题范围

- TOG 官方定位为 computer graphics 旗舰期刊，也是 SIGGRAPH/SIGGRAPH Asia 论文主要出版载体。
- 适合 rendering、modeling、animation、simulation、geometry、imaging、visual computing、graphics systems 和交互创作工具。

### 官方投稿与评审要求

- 论文应有图形学技术新意、算法/系统细节、视觉质量和定量/定性评估。
- TOG 稿件通常要求高度完整、可复现，并说明与图形学社区的关系。

### 给智能体的硬约束

- TOG fit 要求 graphics/visual computing 贡献。
- 普通 CV/ML 结果没有图形生成、渲染、建模或交互技术贡献时 fit 弱。

## 智能体提示词片段

```text
目标 venue 是 ACM Transactions on Graphics (TOG)，CCF-A，类型为期刊，所属 CCF 专业领域为：计算机图形学与多媒体 / Computer Graphics and Multimedia。
