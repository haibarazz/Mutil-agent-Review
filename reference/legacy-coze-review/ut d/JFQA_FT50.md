# Journal of Financial and Quantitative Analysis (JFQA)

> 智能体期刊画像版，更新日期：2026-05-25。资料主要来自 JFQA 官方站与 Cambridge journal page。此文件用于“用户选择期刊后，智能体判断内容 fit、主题 fit 和写作方向”。

## 基本识别

- 简写：JFQA
- 全名：Journal of Financial and Quantitative Analysis
- 榜单归属：FT50
- 主要领域：Financial Economics；Corporate Finance；Investments；Capital Markets；Security Markets；Quantitative Methods
- 出版方：Cambridge University Press
- 官方主页：https://jfqa.org/
- Cambridge page：https://www.cambridge.org/core/journals/journal-of-financial-and-quantitative-analysis
- 投稿入口：https://jfqa.org/submissions/

## 一句话定位

JFQA 发表 financial economics 中的理论和经验研究，主题包括 corporate finance、investments、capital and security markets，以及与金融研究者特别相关的 quantitative methods。

## 官方内容定位：适合什么主题

JFQA 的核心是 financial and quantitative analysis。相较 broad finance，JFQA 对 quantitative methods 和严谨金融实证/理论分析有明显强调。

适合 JFQA 的主题通常包括：

- Corporate finance、资本结构、公司投资、现金、治理、M&A。
- Investments、资产配置、基金、投资者行为。
- Capital markets and security markets。
- 金融计量、统计、机器学习或量化方法，但必须与金融研究问题紧密相关。
- 理论与经验 financial economics。
- 能为金融研究者提供方法、证据或理论 insight 的研究。

## 官方偏好的论文类型与贡献方式

JFQA 的关键是 theoretical and empirical research in financial economics，以及 quantitative methods of relevance to financial researchers。

可接受的贡献方式包括：

- **Theoretical contribution**：金融经济学理论。
- **Empirical contribution**：严谨实证金融研究。
- **Quantitative-method contribution**：对金融研究者特别有用的量化方法。
- **Market contribution**：资本和证券市场机制。
- **Corporate/investment contribution**：公司金融或投资研究。

## 对主题 fit 的判断规则

适合 JFQA 的论文通常满足：

- 研究问题属于 financial economics。
- 理论或经验分析足够清楚和严谨。
- 如果是方法论文，必须对金融研究者有特别相关性。
- 主题落在公司金融、投资、资本市场或证券市场。
- 贡献不是单纯预测性能，而是金融机制或量化金融知识。

不适合 JFQA 的情况：

- 只是机器学习/统计方法展示，没有金融问题。
- 只是财务会计或企业管理问题，金融经济学贡献弱。
- 只是单一市场描述，缺少理论或实证金融 insight。
- 方法复杂但金融研究者价值不清。

## 智能体提示词片段

当用户选择 JFQA 时，可以把下面内容加入系统提示词：

```text
目标期刊是 Journal of Financial and Quantitative Analysis (JFQA)。
请用金融与量化分析顶刊标准评估论文：JFQA 发表 theoretical and empirical research in financial economics，主题包括 corporate finance、investments、capital and security markets，以及对金融研究者特别相关的 quantitative methods。

优先检查：
1. 问题是否属于 financial economics。
2. 是否在 corporate finance、investments、capital/security markets 或 quantitative methods。
3. 方法或理论是否对金融研究者有价值。
4. 论文是否产生金融机制 insight，而不是单纯预测。
5. 实证和量化分析是否严谨。
```

## 来源

- JFQA official site：https://jfqa.org/
- Cambridge JFQA page：https://www.cambridge.org/core/journals/journal-of-financial-and-quantitative-analysis
- JFQA submissions：https://jfqa.org/submissions/
