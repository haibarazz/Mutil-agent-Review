...
- 简写：ISCA
- 全名：International Symposium on Computer Architecture
- 文件名：ISCA_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
ISCA 是 CCF 推荐目录中 **计算机体系结构/并行与分布计算/存储系统** 领域的 A 类会议。计算机体系结构、并行与分布计算、存储系统方向的顶级 venue，适合硬件/系统/存储/HPC/云基础设施等底层计算系统研究。
...
- 论文贡献与 International Symposium on Computer Architecture 的全名和领域传统一致。
...
- ISCA 官方定位为 computer architecture 旗舰会议，关注计算系统硬件架构及其软硬协同。
- 适合处理器、内存层次、互连、accelerators、datacenter architecture、security/reliability、emerging technologies、architecture for AI/HPC/systems。

#### 官方投稿与评审要求

- 论文需要提出有意义的 architecture contribution，并用模拟器、RTL/FPGA/原型、硅数据或严谨建模支撑。
- 评审会关注 novelty、quantitative evaluation、可实现性、成本/能耗/性能权衡和架构影响。

#### 给智能体的硬约束

- ISCA fit 必须围绕架构机制或架构设计空间。
- 纯软件系统、编译器优化或应用 benchmark 没有架构 insight 时 fit 弱。
...
```text
目标 venue 是 International Symposium on Computer Architecture (ISCA)，CCF-A，类型为会议，所属 CCF 专业领域为：计算机体系结构/并行与分布计算/存储系统 / Computer Architecture / Parallel and Distributed Computing / Storage System。
