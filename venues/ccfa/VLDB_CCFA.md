...
- 简写：VLDB
- 全名：International Conference on Very Large Data Bases
- 文件名：VLDB_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
- CCF 专业领域：数据库/数据挖掘/内容检索 / Database / Data Mining / Content Retrieval
- 出版方 / 主办方：Morgan Kaufmann/ACM
- CCF 官方目录入口：https://www.ccf.org.cn/Academic_Evaluation/By_category/

## 一句话定位

VLDB 是 CCF 推荐目录中 **数据库/数据挖掘/内容检索** 领域的 A 类会议。数据库、数据挖掘和内容检索方向的顶级 venue，适合数据管理、数据系统、数据挖掘、信息检索和 Web 数据研究。
...
- 论文贡献与 International Conference on Very Large Data Bases 的全名和领域传统一致。
...
- VLDB 2026: https://www.vldb.org/2026/

来源类型：官方 CFP / 官方会议页面

### 官方定位与主题范围

- VLDB 官方定位为 very large data bases 和 data management 顶级会议/期刊式 proceedings。
- 适合大规模数据管理、数据库系统、查询、事务、分布式数据、图/流/时序、数据集成、数据治理、AI data systems。

### 官方投稿与评审要求

- VLDB 论文应有可扩展数据管理贡献，通常需要系统实现、真实 workload、大规模实验或理论分析。
- PVLDB 风格强调完整性、可复现性、实践价值和清晰的数据管理定位。

### 给智能体的硬约束

- VLDB fit 必须落在 large-scale data management。
- 只使用大数据但不研究数据系统/管理问题，fit 弱。

## 智能体提示词片段

```text
目标 venue 是 International Conference on Very Large Data Bases (VLDB)，CCF-A，类型为会议，所属 CCF 专业领域为：数据库/数据挖掘/内容检索 / Database / Data Mining / Content Retrieval。
