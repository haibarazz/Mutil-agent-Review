...
- 简写：TOCS
- 全名：ACM Transactions on Computer Systems
- 文件名：TOCS_CCFA.md
- 类型：期刊
- 榜单归属：CCF-A
...
TOCS 是 CCF 推荐目录中 **计算机体系结构/并行与分布计算/存储系统** 领域的 A 类期刊。计算机体系结构、并行与分布计算、存储系统方向的顶级 venue，适合硬件/系统/存储/HPC/云基础设施等底层计算系统研究。
...
- 论文贡献与 ACM Transactions on Computer Systems 的全名和领域传统一致。
...
- ACM TOCS About: https://dl.acm.org/journal/tocs/about

来源类型：官方 Aims & Scope / About / Author 页面

### 官方定位与主题范围

- TOCS 官方定位为 computer systems 期刊，关注系统设计、实现、行为和使用。
- 适合 OS、distributed systems、storage、networked systems、cloud/datacenter、security/reliability、system measurement 等。

### 官方投稿与评审要求

- 论文应有完整系统贡献、严谨实现和评估，解释设计 trade-off 和可推广系统 insight。
- 期刊稿件需要充分展开方法、实验、边界条件和与已有系统的比较。

### 给智能体的硬约束

- TOCS fit 要求系统研究核心贡献。
- 只有应用层 feature、缺乏系统机制或真实评估时 fit 弱。

## 智能体提示词片段

```text
目标 venue 是 ACM Transactions on Computer Systems (TOCS)，CCF-A，类型为期刊，所属 CCF 专业领域为：计算机体系结构/并行与分布计算/存储系统 / Computer Architecture / Parallel and Distributed Computing / Storage System。
