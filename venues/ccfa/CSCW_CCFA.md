...
- 简写：CSCW
- 全名：ACM Conference On Computer-Supported Cooperative Work And Social Computing
- 文件名：CSCW_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
CSCW 是 CCF 推荐目录中 **人机交互与普适计算** 领域的 A 类会议。人机交互与普适计算方向的顶级 venue，适合 HCI、CSCW、交互系统、普适计算、可用性和社会计算研究。
...
- 论文贡献与 ACM Conference On Computer-Supported Cooperative Work And Social Computing 的全名和领域传统一致。
...
- CSCW 官方定位为 computer-supported cooperative work and social computing。
- 适合协作系统、社交计算、在线社区、组织协作、平台治理、群体行为、社会技术系统、AI-mediated collaboration。

### 官方投稿与评审要求

- 论文应有社会技术问题、理论/系统/设计/实证贡献，并使用合适的定性、定量、计算或设计研究方法。
- CSCW 重视伦理、参与者/社区影响、平台情境和研究方法透明度。

### 给智能体的硬约束

- CSCW fit 要求 collaboration/social computing 核心。
- 只分析社交数据但没有社会技术研究问题或协作洞察时 fit 弱。

## 智能体提示词片段

```text
目标 venue 是 ACM Conference On Computer-Supported Cooperative Work And Social Computing (CSCW)，CCF-A，类型为会议，所属 CCF 专业领域为：人机交互与普适计算 / Human-Computer Interaction and Pervasive Computing。
