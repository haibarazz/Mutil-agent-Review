...
- 简写：LICS
- 全名：ACM/IEEE Symposium on Logic in Computer Science
- 文件名：LICS_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
LICS 是 CCF 推荐目录中 **计算机科学理论** 领域的 A 类会议。计算机科学理论方向的顶级 venue，适合算法、复杂性、计算理论、密码理论、逻辑和理论计算机科学研究。
...
- 论文贡献与 ACM/IEEE Symposium on Logic in Computer Science 的全名和领域传统一致。
...
- LICS 官方定位为 logic in computer science。
- 适合逻辑、语义、类型理论、验证、自动推理、数据库逻辑、复杂性、范畴/证明理论、程序逻辑。

### 官方投稿与评审要求

- 论文应有严谨逻辑/理论贡献，包含定义、定理和证明，或逻辑方法在 CS 中的深层应用。
- 评审关注形式化正确性、理论新意和与逻辑/CS 的关系。

### 给智能体的硬约束

- LICS fit 要求 logic/formal theory。
- 只有工具或经验实验而无逻辑贡献时 fit 弱。

## 智能体提示词片段

```text
目标 venue 是 ACM/IEEE Symposium on Logic in Computer Science (LICS)，CCF-A，类型为会议，所属 CCF 专业领域为：计算机科学理论 / Computer Science Theory。
