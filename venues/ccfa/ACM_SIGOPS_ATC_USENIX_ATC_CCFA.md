# ACM SIGOPS Annual Technical Conference (ACM SIGOPS ATC （原 USENIX ATC）)
...
- 简写：ACM SIGOPS ATC （原 USENIX ATC）
- 全名：ACM SIGOPS Annual Technical Conference
- 文件名：ACM_SIGOPS_ATC_USENIX_ATC_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
ACM SIGOPS ATC （原 USENIX ATC） 是 CCF 推荐目录中 **计算机体系结构/并行与分布计算/存储系统** 领域的 A 类会议。计算机体系结构、并行与分布计算、存储系统方向的顶级 venue，适合硬件/系统/存储/HPC/云基础设施等底层计算系统研究。
...
- ACM SIGOPS ATC 2026: https://sigops.org/s/conferences/atc/2026/index.html

来源类型：官方 CFP / 官方会议页面

#### 官方定位与主题范围

- 官方定位为 systems research 顶级会议，继承 USENIX ATC 的 broad systems tradition，关注真实计算系统的设计、实现、评估和经验。
- 适合操作系统、分布式系统、云/数据中心、虚拟化、存储、网络、可靠性、安全、性能、可观测性、资源管理等系统主题。

#### 官方投稿与评审要求

- 论文应提供清晰系统问题、可运行系统或原型、充分实验评估、真实 workload 或可信模拟，并解释设计权衡。
- 系统类贡献通常需要超过工程实现本身，给出可迁移的设计原则、机制洞察、性能/可靠性/可维护性证据。

#### 给智能体的硬约束

- 不要把普通工程优化包装成 ATC；必须说明系统问题为何普遍、设计为何新、评估为何能支撑结论。
- 若只是应用层功能或单机小实验，fit 弱。
...
```text
目标 venue 是 ACM SIGOPS Annual Technical Conference (ACM SIGOPS ATC （原 USENIX ATC）)，CCF-A，类型为会议，所属 CCF 专业领域为：计算机体系结构/并行与分布计算/存储系统 / Computer Architecture / Parallel and Distributed Computing / Storage System。
...
- 如果审稿人认为这是相邻领域论文，你如何解释它适合 ACM SIGOPS ATC （原 USENIX ATC）？
