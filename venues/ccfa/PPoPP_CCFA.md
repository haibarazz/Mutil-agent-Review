...
- 简写：PPoPP
- 全名：ACM SIGPLAN Symposium on Principles & Practice of Parallel Programming
- 文件名：PPoPP_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
PPoPP 是 CCF 推荐目录中 **计算机体系结构/并行与分布计算/存储系统** 领域的 A 类会议。计算机体系结构、并行与分布计算、存储系统方向的顶级 venue，适合硬件/系统/存储/HPC/云基础设施等底层计算系统研究。
...
- 论文贡献与 ACM SIGPLAN Symposium on Principles & Practice of Parallel Programming 的全名和领域传统一致。
...
- PPoPP 2026: https://ppopp26.sigplan.org/

来源类型：官方 CFP / 官方会议页面

#### 官方定位与主题范围

- PPoPP 官方定位为 principles and practice of parallel programming，连接并行编程语言、编译器、运行时和并行系统。
- 适合并行语言/模型、编译优化、runtime、scheduling、GPU/accelerator programming、并发正确性、性能分析、parallel algorithms in systems context。

#### 官方投稿与评审要求

- 论文应同时体现 principle 与 practice：清晰抽象或方法，并通过实现/实验/理论说明并行性能、可移植性或可编程性。
- 评审关注 parallel programming 问题的重要性、方法严谨性、实现可用性和性能证据。

#### 给智能体的硬约束

- PPoPP fit 不只是并行跑得快，而是对并行编程模型、编译/运行时或实践有贡献。
- 纯应用并行化且无可迁移方法时 fit 弱。
...
```text
目标 venue 是 ACM SIGPLAN Symposium on Principles & Practice of Parallel Programming (PPoPP)，CCF-A，类型为会议，所属 CCF 专业领域为：计算机体系结构/并行与分布计算/存储系统 / Computer Architecture / Parallel and Distributed Computing / Storage System。
...
- 如果审稿人认为这是相邻领域论文，你如何解释它适合 PPoPP？
