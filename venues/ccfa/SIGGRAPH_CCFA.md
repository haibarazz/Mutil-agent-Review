...
- 简写：SIGGRAPH
- 全名：ACM Special Interest Group on Computer Graphics
- 文件名：SIGGRAPH_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
SIGGRAPH 是 CCF 推荐目录中 **计算机图形学与多媒体** 领域的 A 类会议。计算机图形学与多媒体方向的顶级 venue，适合图形、视觉化、多媒体、VR/AR、渲染和交互媒体研究。
...
- 论文贡献与 ACM Special Interest Group on Computer Graphics 的全名和领域传统一致。
...
- SIGGRAPH 官方定位为 computer graphics and interactive techniques 顶级会议。
- 适合 rendering、geometry、animation、simulation、imaging、graphics systems、AR/VR、creative tools、visual computing。

#### 官方投稿与评审要求

- 技术论文应提供图形学/交互技术新贡献，通常需要数学/算法/系统细节、视觉质量、性能和比较评估。
- 评审关注 novelty、technical soundness、visual/interactive impact 和 reproducibility。

#### 给智能体的硬约束

- SIGGRAPH fit 要求 graphics/interactive techniques 核心贡献。
- 普通视觉识别或多媒体检索没有图形学贡献时 fit 弱。
...
```text
目标 venue 是 ACM Special Interest Group on Computer Graphics (SIGGRAPH)，CCF-A，类型为会议，所属 CCF 专业领域为：计算机图形学与多媒体 / Computer Graphics and Multimedia。
...
- 如果审稿人认为这是相邻领域论文，你如何解释它适合 SIGGRAPH？
