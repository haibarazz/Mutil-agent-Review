...
- 简写：EUROCRYPT
- 全名：International Conference on the Theory and Applications of Cryptographic Techniques
- 文件名：EUROCRYPT_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
- CCF 专业领域：网络与信息安全 / Network and Information Security
- 出版方 / 主办方：Springer
- CCF 官方目录入口：https://www.ccf.org.cn/Academic_Evaluation/By_category/

### 一句话定位

EUROCRYPT 是 CCF 推荐目录中 **网络与信息安全** 领域的 A 类会议。网络与信息安全方向的顶级 venue，适合密码学、系统安全、网络安全、隐私、安全攻防和安全协议研究。

### 官方目录定位：适合什么主题

CCF 第七版目录把该 venue 归入 **网络与信息安全**，并标记为 **A 类**。对智能体来说，最重要的 fit 判断是：论文是否真正推进该 CCF 专业领域的核心问题，而不是只借用该领域作为应用背景。
...
适合 EUROCRYPT 的论文通常满足：

- 研究问题属于 **网络与信息安全** 的核心问题。
- 论文贡献与 International Conference on the Theory and Applications of Cryptographic Techniques 的全名和领域传统一致。
...
- EUROCRYPT 2026: https://eurocrypt.iacr.org/2026/

来源类型：官方 CFP / 官方会议页面

#### 官方定位与主题范围

- EUROCRYPT 官方定位为 cryptographic theory and applications 顶级会议。
- 适合公钥/对称密码、协议、安全证明、复杂性、后量子、零知识、多方计算、隐私增强密码技术等。

#### 官方投稿与评审要求

- 论文应提供清晰密码学问题、正式模型和严谨证明，或对密码系统的实质分析。
- 评审关注 novelty、proof correctness、assumptions、security reductions 和与已有密码工作的区别。

#### 给智能体的硬约束

- EUROCRYPT fit 与 CRYPTO 类似，必须是 cryptography research。
- 没有形式化安全论证的工程系统一般不适合。
...
```text
目标 venue 是 International Conference on the Theory and Applications of Cryptographic Techniques (EUROCRYPT)，CCF-A，类型为会议，所属 CCF 专业领域为：网络与信息安全 / Network and Information Security。

请按 CCF-A 顶级 CS venue 标准评估论文：先判断研究问题是否真正属于该专业领域，再检查贡献是否达到顶级会议要求。重点评估问题重要性、技术/理论/系统新意、实验或证明强度、与强基线对比、可复现性、局限性和领域影响。

如果论文只是把通用方法套到一个场景，或者主题与 网络与信息安全 关系弱，请明确指出 fit 不足，并建议如何重写为该领域核心问题。
```

### 可用于提问用户的 fit-check 问题

- 你的论文最核心的 CS 问题是什么？它为什么属于 **网络与信息安全**？
...
- 如果审稿人认为这是相邻领域论文，你如何解释它适合 EUROCRYPT？
