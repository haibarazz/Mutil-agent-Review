...
- 简写：CAV
- 全名：International Conference on Computer Aided Verification
- 文件名：CAV_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
- CCF 专业领域：计算机科学理论 / Computer Science Theory
- 出版方 / 主办方：Springer
- CCF 官方目录入口：https://www.ccf.org.cn/Academic_Evaluation/By_category/

## 一句话定位

CAV 是 CCF 推荐目录中 **计算机科学理论** 领域的 A 类会议。计算机科学理论方向的顶级 venue，适合算法、复杂性、计算理论、密码理论、逻辑和理论计算机科学研究。

## 官方目录定位：适合什么主题

CCF 第七版目录把该 venue 归入 **计算机科学理论**，并标记为 **A 类**。对智能体来说，最重要的 fit 判断是：论文是否真正推进该 CCF 专业领域的核心问题，而不是只借用该领域作为应用背景。
...
- 研究问题属于 **计算机科学理论** 的核心问题。
- 论文贡献与 International Conference on Computer Aided Verification 的全名和领域传统一致。
...
- CAV 官方定位为 computer-aided verification。
- 适合 model checking、SMT/SAT、formal verification、program/hardware verification、hybrid systems、security verification、AI systems verification、tools。

### 官方投稿与评审要求

- 论文应有验证方法、理论、工具或应用案例，并说明 soundness、completeness、scalability 或 practical impact。
- CAV 重视 tool papers 和 artifact，但必须有形式化验证贡献。

### 给智能体的硬约束

- CAV fit 要求 verification/formal methods 核心。
- 普通测试、经验分析或安全扫描没有形式化验证贡献时 fit 弱。

## 智能体提示词片段

```text
目标 venue 是 International Conference on Computer Aided Verification (CAV)，CCF-A，类型为会议，所属 CCF 专业领域为：计算机科学理论 / Computer Science Theory。

请按 CCF-A 顶级 CS venue 标准评估论文：先判断研究问题是否真正属于该专业领域，再检查贡献是否达到顶级会议要求。重点评估问题重要性、技术/理论/系统新意、实验或证明强度、与强基线对比、可复现性、局限性和领域影响。

如果论文只是把通用方法套到一个场景，或者主题与 计算机科学理论 关系弱，请明确指出 fit 不足，并建议如何重写为该领域核心问题。
```

## 可用于提问用户的 fit-check 问题

- 你的论文最核心的 CS 问题是什么？它为什么属于 **计算机科学理论**？
