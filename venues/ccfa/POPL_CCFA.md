...
- 简写：POPL
- 全名：ACM SIGPLAN-SIGACT Symposium on Principles of Programming Languages
- 文件名：POPL_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
POPL 是 CCF 推荐目录中 **软件工程/系统软件/程序设计语言** 领域的 A 类会议。软件工程、系统软件和程序设计语言方向的顶级 venue，适合软件开发、程序分析、编译器、操作系统、PL 和形式化方法研究。
...
- 论文贡献与 ACM SIGPLAN-SIGACT Symposium on Principles of Programming Languages 的全名和领域传统一致。
...
- POPL 官方定位为 principles of programming languages。
- 适合语义、类型系统、程序逻辑、验证、编译理论、语言设计原则、并发/概率/量子语言基础等。

### 官方投稿与评审要求

- 论文通常需要强形式化内容，包括定义、定理、证明、语义模型或严谨语言机制。
- 实现可以加分，但核心是原则性和理论贡献。

### 给智能体的硬约束

- POPL fit 要求 PL principles。
- 只有工程实现而无形式化/理论或语言原则贡献时 fit 弱。

## 智能体提示词片段

```text
目标 venue 是 ACM SIGPLAN-SIGACT Symposium on Principles of Programming Languages (POPL)，CCF-A，类型为会议，所属 CCF 专业领域为：软件工程/系统软件/程序设计语言 / Software Engineering / System Software / Programming Languages。
