...
- 简写：MICRO
- 全名：IEEE/ACM International Symposium on Microarchitecture
- 文件名：MICRO_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
MICRO 是 CCF 推荐目录中 **计算机体系结构/并行与分布计算/存储系统** 领域的 A 类会议。计算机体系结构、并行与分布计算、存储系统方向的顶级 venue，适合硬件/系统/存储/HPC/云基础设施等底层计算系统研究。
...
- 论文贡献与 IEEE/ACM International Symposium on Microarchitecture 的全名和领域传统一致。
...
- MICRO 2026: https://microarch.org/micro59/

来源类型：官方 CFP / 官方会议页面

#### 官方定位与主题范围

- MICRO 官方定位为 microarchitecture 顶级会议，聚焦处理器和计算系统微架构。
- 适合 pipelines、out-of-order、cache/memory、branch prediction、accelerators、memory consistency、security/reliability、ML for microarchitecture、emerging devices。

#### 官方投稿与评审要求

- 论文需证明微架构机制的性能、能耗、面积、复杂度和正确性影响，评估需有标准 benchmark 与强 baseline。
- 好的 MICRO 论文通常给出细粒度机制 insight，而不是只给系统级吞吐量。

#### 给智能体的硬约束

- MICRO fit 要求贡献落在 microarchitecture 层。
- 如果无法说明硬件机制或微架构实现路径，应判为弱 fit。
...
```text
目标 venue 是 IEEE/ACM International Symposium on Microarchitecture (MICRO)，CCF-A，类型为会议，所属 CCF 专业领域为：计算机体系结构/并行与分布计算/存储系统 / Computer Architecture / Parallel and Distributed Computing / Storage System。
...
- 如果审稿人认为这是相邻领域论文，你如何解释它适合 MICRO？
