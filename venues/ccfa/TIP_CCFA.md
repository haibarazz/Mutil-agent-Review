...
- 简写：TIP
- 全名：IEEE Transactions on Image Processing
- 文件名：TIP_CCFA.md
- 类型：期刊
- 榜单归属：CCF-A
...
TIP 是 CCF 推荐目录中 **计算机图形学与多媒体** 领域的 A 类期刊。计算机图形学与多媒体方向的顶级 venue，适合图形、视觉化、多媒体、VR/AR、渲染和交互媒体研究。
...
- 论文贡献与 IEEE Transactions on Image Processing 的全名和领域传统一致。
...
- TIP 官方定位为 image processing 旗舰期刊。
- 适合图像/视频/视觉信号处理、恢复、增强、压缩、分析、生成、计算成像和相关理论。

#### 官方投稿与评审要求

- 论文应有图像处理方法、理论或系统贡献，并用强 baseline、数据集、指标和分析支撑。
- 期刊稿件需要充分实验、消融、复杂度和局限讨论。

#### 给智能体的硬约束

- TIP fit 要求 image processing 问题为核心。
- 纯高层识别或通用 ML 若无图像处理贡献，fit 弱。
...
```text
目标 venue 是 IEEE Transactions on Image Processing (TIP)，CCF-A，类型为期刊，所属 CCF 专业领域为：计算机图形学与多媒体 / Computer Graphics and Multimedia。
