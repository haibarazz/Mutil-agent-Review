...
- 简写：TMC
- 全名：IEEE Transactions on Mobile Computing
- 文件名：TMC_CCFA.md
- 类型：期刊
- 榜单归属：CCF-A
...
TMC 是 CCF 推荐目录中 **计算机网络** 领域的 A 类期刊。计算机网络方向的顶级 venue，适合网络协议、网络系统、移动/无线网络、互联网测量和网络体系结构研究。
...
- 研究问题属于 **计算机网络** 的核心问题。
- 论文贡献与 IEEE Transactions on Mobile Computing 的全名和领域传统一致。
...
- TMC 官方定位为 mobile computing 期刊，覆盖移动、无线、普适和边缘计算系统。
- 适合 mobile systems、wireless networking、mobile sensing、IoT、edge computing、mobility management、privacy/security、energy-aware computing。

#### 官方投稿与评审要求

- 论文应围绕移动性、无线环境、设备约束、用户行为、能耗或移动网络系统提供原创贡献。
- 需要真实数据、原型、部署、仿真或理论分析支撑。

#### 给智能体的硬约束

- TMC fit 不等于“运行在手机上”；必须有 mobile computing 研究问题。
- 缺少移动/无线/边缘约束时 fit 弱。
...
```text
目标 venue 是 IEEE Transactions on Mobile Computing (TMC)，CCF-A，类型为期刊，所属 CCF 专业领域为：计算机网络 / Computer Networks。
