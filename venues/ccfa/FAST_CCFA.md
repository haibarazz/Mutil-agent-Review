...
- 简写：FAST
- 全名：USENIX Conference on File and Storage Technologies
- 文件名：FAST_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
FAST 是 CCF 推荐目录中 **计算机体系结构/并行与分布计算/存储系统** 领域的 A 类会议。计算机体系结构、并行与分布计算、存储系统方向的顶级 venue，适合硬件/系统/存储/HPC/云基础设施等底层计算系统研究。
...
- 论文贡献与 USENIX Conference on File and Storage Technologies 的全名和领域传统一致。
...
- FAST 2026: https://www.usenix.org/conference/fast26

来源类型：官方 CFP / 官方会议页面

#### 官方定位与主题范围

- FAST 官方定位为 file and storage technologies 顶级会议，聚焦存储系统、文件系统、持久化、数据管理和可靠性。
- 适合文件系统、分布式存储、key-value/storage engines、SSD/NVM、备份恢复、容错、数据可靠性、存储安全、性能隔离、存储 for ML/HPC/cloud 等。

#### 官方投稿与评审要求

- 论文应围绕存储问题提出机制、系统、测量或实践经验，并用真实 workload、trace、故障模型或生产经验支撑。
- 评审通常重视系统完整性、可靠性语义、性能/成本权衡和数据安全/一致性。

#### 给智能体的硬约束

- FAST fit 必须有明确 storage/file-system/data persistence 核心问题。
- 只把数据放在存储上但不研究存储系统本身，fit 弱。
...
```text
目标 venue 是 USENIX Conference on File and Storage Technologies (FAST)，CCF-A，类型为会议，所属 CCF 专业领域为：计算机体系结构/并行与分布计算/存储系统 / Computer Architecture / Parallel and Distributed Computing / Storage System。
