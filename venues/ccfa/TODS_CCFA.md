...
- 简写：TODS
- 全名：ACM Transactions on Database Systems
- 文件名：TODS_CCFA.md
- 类型：期刊
- 榜单归属：CCF-A
...
TODS 是 CCF 推荐目录中 **数据库/数据挖掘/内容检索** 领域的 A 类期刊。数据库、数据挖掘和内容检索方向的顶级 venue，适合数据管理、数据系统、数据挖掘、信息检索和 Web 数据研究。
...
- 论文贡献与 ACM Transactions on Database Systems 的全名和领域传统一致。
...
- ACM TODS About: https://dl.acm.org/journal/tods/about

来源类型：官方 Aims & Scope / About / Author 页面

### 官方定位与主题范围

- TODS 官方定位为 database systems 旗舰期刊。
- 适合数据库理论与系统、查询处理、事务、并发控制、恢复、数据模型、分布式数据库、数据管理算法。

### 官方投稿与评审要求

- 论文应提供完整数据库贡献，包含正确性、复杂度、系统实现或严谨实验。
- 期刊版本需要完整理论和/或系统评价，而非短期工程结果。

### 给智能体的硬约束

- TODS fit 要求 database/data management 核心。
- 普通数据分析或 ML 应用没有 DB 贡献时 fit 弱。

## 智能体提示词片段

```text
目标 venue 是 ACM Transactions on Database Systems (TODS)，CCF-A，类型为期刊，所属 CCF 专业领域为：数据库/数据挖掘/内容检索 / Database / Data Mining / Content Retrieval。
