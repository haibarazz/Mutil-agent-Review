...
- 简写：EuroSys
- 全名：European Conference on Computer Systems
- 文件名：EuroSys_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
EuroSys 是 CCF 推荐目录中 **计算机体系结构/并行与分布计算/存储系统** 领域的 A 类会议。计算机体系结构、并行与分布计算、存储系统方向的顶级 venue，适合硬件/系统/存储/HPC/云基础设施等底层计算系统研究。
...
- 论文贡献与 European Conference on Computer Systems 的全名和领域传统一致。
...
- EuroSys 官方定位为欧洲系统会议，关注 computer systems research 的设计、实现、测量和分析。
- 适合操作系统、分布式系统、云、边缘、存储、网络、运行时、系统安全、可靠性、资源管理、系统 for ML 等。

### 官方投稿与评审要求

- 论文需要有系统贡献和严谨评估，通常要求真实系统、开源/可复现工件或充分的 workload/trace 实验。
- EuroSys 强调系统设计的原则、trade-off 和可推广 insight，而非单一场景性能数字。

### 给智能体的硬约束

- 判断 EuroSys fit 时优先问：这是否是一个系统问题，是否有系统机制，是否有真实评估。
- 只有算法模型而无系统实现或系统洞察时 fit 弱。

## 智能体提示词片段

```text
目标 venue 是 European Conference on Computer Systems (EuroSys)，CCF-A，类型为会议，所属 CCF 专业领域为：计算机体系结构/并行与分布计算/存储系统 / Computer Architecture / Parallel and Distributed Computing / Storage System。
...
- 如果审稿人认为这是相邻领域论文，你如何解释它适合 EuroSys？
