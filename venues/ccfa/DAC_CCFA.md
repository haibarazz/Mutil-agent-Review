...
- 简写：DAC
- 全名：Design Automation Conference
- 文件名：DAC_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
DAC 是 CCF 推荐目录中 **计算机体系结构/并行与分布计算/存储系统** 领域的 A 类会议。计算机体系结构、并行与分布计算、存储系统方向的顶级 venue，适合硬件/系统/存储/HPC/云基础设施等底层计算系统研究。
...
- 论文贡献与 Design Automation Conference 的全名和领域传统一致。
...
- DAC 官方定位为 electronic design automation 和 design automation 旗舰会议，覆盖电子系统、集成电路、EDA 方法和设计流程。
- 适合系统级、逻辑级、物理级设计，综合、验证、测试、布局布线、硬软协同、嵌入式系统、设计安全、AI/ML for EDA 等主题。

#### 官方投稿与评审要求

- 论文应体现对设计自动化或电子系统设计流程的实质推进，通常需要工业相关 benchmark、设计质量指标、可扩展性和与现有 EDA 方法对比。
- 实践/工具类论文需说明其在真实设计复杂度、PPA、验证效率或生产流程中的价值。

#### 给智能体的硬约束

- DAC fit 不是泛硬件或泛算法，必须落到 design automation / EDA / electronic systems design。
- 如果贡献无法影响设计流程、验证、综合、测试或实现质量，应判为弱 fit。
...
```text
目标 venue 是 Design Automation Conference (DAC)，CCF-A，类型为会议，所属 CCF 专业领域为：计算机体系结构/并行与分布计算/存储系统 / Computer Architecture / Parallel and Distributed Computing / Storage System。
