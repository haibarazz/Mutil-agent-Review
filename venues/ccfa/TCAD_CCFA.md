...
- 简写：TCAD
- 全名：IEEE Transactions on Computer-Aided Design of Integrated Circuits and Systems
- 文件名：TCAD_CCFA.md
- 类型：期刊
- 榜单归属：CCF-A
...
TCAD 是 CCF 推荐目录中 **计算机体系结构/并行与分布计算/存储系统** 领域的 A 类期刊。计算机体系结构、并行与分布计算、存储系统方向的顶级 venue，适合硬件/系统/存储/HPC/云基础设施等底层计算系统研究。
...
- 论文贡献与 IEEE Transactions on Computer-Aided Design of Integrated Circuits and Systems 的全名和领域传统一致。
...
- IEEE CEDA TCAD: https://ieee-ceda.org/publications/tcad

来源类型：官方 Aims & Scope / About / Author 页面

### 官方定位与主题范围

- TCAD 官方定位为 computer-aided design of integrated circuits and systems，关注 IC/system design 的自动化方法。
- 适合 planning、synthesis、partitioning、modeling、simulation、layout、verification、testing、hardware-software co-design、design documentation 等。

### 官方投稿与评审要求

- 论文应提出 CAD/EDA 方法、模型、算法或界面，并在复杂设计场景中验证质量、效率、可扩展性和正确性。
- 需要与现有 EDA flow 或工业 benchmark 建立联系。

### 给智能体的硬约束

- TCAD fit 必须服务于 IC/system design automation。
- 若只是电路结果或 ML 方法，没有 CAD 任务和设计流程价值，fit 弱。

## 智能体提示词片段

```text
目标 venue 是 IEEE Transactions on Computer-Aided Design of Integrated Circuits and Systems (TCAD)，CCF-A，类型为期刊，所属 CCF 专业领域为：计算机体系结构/并行与分布计算/存储系统 / Computer Architecture / Parallel and Distributed Computing / Storage System。
