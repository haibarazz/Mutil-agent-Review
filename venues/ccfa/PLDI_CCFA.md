...
- 简写：PLDI
- 全名：ACM SIGPLAN Conference on Programming Language Design and Implementation
- 文件名：PLDI_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
PLDI 是 CCF 推荐目录中 **软件工程/系统软件/程序设计语言** 领域的 A 类会议。软件工程、系统软件和程序设计语言方向的顶级 venue，适合软件开发、程序分析、编译器、操作系统、PL 和形式化方法研究。
...
- 论文贡献与 ACM SIGPLAN Conference on Programming Language Design and Implementation 的全名和领域传统一致。
...
- PLDI 官方定位为 programming language design and implementation。
- 适合编译器、程序分析、语言设计、类型系统、运行时、verification、optimization、DSL、parallel/GPU languages。

#### 官方投稿与评审要求

- 论文应提出 PL/编译/运行时贡献，并用理论证明、实现、benchmark 或实证研究支撑。
- 评审关注 correctness、performance、language usability、implementation quality 和与已有 PL 工作的关系。

#### 给智能体的硬约束

- PLDI fit 必须有 programming languages 或 compiler/runtime 核心。
- 纯软件工程工具或系统优化若无 PL 贡献，fit 弱。
...
```text
目标 venue 是 ACM SIGPLAN Conference on Programming Language Design and Implementation (PLDI)，CCF-A，类型为会议，所属 CCF 专业领域为：软件工程/系统软件/程序设计语言 / Software Engineering / System Software / Programming Languages。
