# ACM International Conference on Applications, Technologies, Architectures, and Protocols for Computer Communication (SIGCOMM)
...
- 简写：SIGCOMM
- 全名：ACM International Conference on Applications, Technologies, Architectures, and Protocols for Computer Communication
- 文件名：SIGCOMM_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
- CCF 专业领域：计算机网络 / Computer Networks
- 出版方 / 主办方：ACM
- CCF 官方目录入口：https://www.ccf.org.cn/Academic_Evaluation/By_category/

## 一句话定位

SIGCOMM 是 CCF 推荐目录中 **计算机网络** 领域的 A 类会议。计算机网络方向的顶级 venue，适合网络协议、网络系统、移动/无线网络、互联网测量和网络体系结构研究。
...
- 研究问题属于 **计算机网络** 的核心问题。
- 论文贡献与 ACM International Conference on Applications, Technologies, Architectures, and Protocols for Computer Communication 的全名和领域传统一致。
...
- SIGCOMM 官方定位为 data communication 和 computer networking 旗舰会议。
- 适合网络架构、协议、测量、拥塞控制、路由、网络验证、无线/数据中心/互联网系统、网络安全和可编程网络。

### 官方投稿与评审要求

- 论文需要有 networking contribution，并以理论、系统、测量或部署证据支撑。
- 强论文通常说明网络层面的新问题、机制或经验，而不只是应用性能。

### 给智能体的硬约束

- SIGCOMM fit 的核心是 communication/networking。
- 如果贡献可以完全脱离网络语境成立，应质疑 fit。

## 智能体提示词片段

```text
目标 venue 是 ACM International Conference on Applications, Technologies, Architectures, and Protocols for Computer Communication (SIGCOMM)，CCF-A，类型为会议，所属 CCF 专业领域为：计算机网络 / Computer Networks。
...
- 如果审稿人认为这是相邻领域论文，你如何解释它适合 SIGCOMM？
