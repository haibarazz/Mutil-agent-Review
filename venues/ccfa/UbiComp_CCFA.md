...
- 简写：UbiComp
- 全名：ACM international joint conference on Pervasive and Ubiquitous Computing
- 文件名：UbiComp_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
UbiComp 是 CCF 推荐目录中 **人机交互与普适计算** 领域的 A 类会议。人机交互与普适计算方向的顶级 venue，适合 HCI、CSCW、交互系统、普适计算、可用性和社会计算研究。
...
- 论文贡献与 ACM international joint conference on Pervasive and Ubiquitous Computing 的全名和领域传统一致。
...
- UbiComp 官方定位为 pervasive and ubiquitous computing。
- 适合 ubiquitous/pervasive systems、wearables、sensing、mobile/edge、context-aware computing、IoT、human activity recognition、smart environments。

#### 官方投稿与评审要求

- 论文通常需要真实传感/部署/用户场景、系统或算法贡献，并说明普适计算约束、隐私和长期可用性。
- UbiComp 与 IMWUT 紧密相关，重视系统/实证完整性和现实场景评估。

#### 给智能体的硬约束

- UbiComp fit 要求 ubiquitous/pervasive computing 问题。
- 普通移动应用或传感分类如果没有场景、部署或人机环境贡献，fit 弱。
...
```text
目标 venue 是 ACM international joint conference on Pervasive and Ubiquitous Computing (UbiComp)，CCF-A，类型为会议，所属 CCF 专业领域为：人机交互与普适计算 / Human-Computer Interaction and Pervasive Computing。
...
- 如果审稿人认为这是相邻领域论文，你如何解释它适合 UbiComp？
