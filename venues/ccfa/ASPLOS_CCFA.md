...
- 简写：ASPLOS
- 全名：International Conference on Architectural Support for Programming Languages and Operating Systems
- 文件名：ASPLOS_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
ASPLOS 是 CCF 推荐目录中 **计算机体系结构/并行与分布计算/存储系统** 领域的 A 类会议。计算机体系结构、并行与分布计算、存储系统方向的顶级 venue，适合硬件/系统/存储/HPC/云基础设施等底层计算系统研究。
...
- 论文贡献与 International Conference on Architectural Support for Programming Languages and Operating Systems 的全名和领域传统一致。
...
- ASPLOS 官方定位在 architecture、programming languages、operating systems 交叉处，强调跨层设计和硬件/软件协同。
- 适合体系结构、编译器、运行时、OS、存储、系统、安全、ML systems、accelerators、quantum/emerging platforms 等跨层工作。

### 官方投稿与评审要求

- 论文通常应清楚说明跨层贡献，不能只属于单一实现细节；需要严谨性能、能耗、可扩展性、正确性或可用性评估。
- 官方 CFP 通常鼓励打破传统领域边界的工作，评审会看问题重要性、设计新颖性、实验严谨性和跨社区影响。

### 给智能体的硬约束

- ASPLOS fit 的关键是 architecture/PL/OS 至少两个层面的耦合贡献。
- 只做模型调参、单一应用优化或缺少硬件/系统上下文的工作，应判为弱 fit。

## 智能体提示词片段

```text
目标 venue 是 International Conference on Architectural Support for Programming Languages and Operating Systems (ASPLOS)，CCF-A，类型为会议，所属 CCF 专业领域为：计算机体系结构/并行与分布计算/存储系统 / Computer Architecture / Parallel and Distributed Computing / Storage System。
...
- 如果审稿人认为这是相邻领域论文，你如何解释它适合 ASPLOS？
