...
- 简写：USENIX Security
- 全名：USENIX Security Symposium
- 文件名：USENIX_Security_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
- CCF 专业领域：网络与信息安全 / Network and Information Security
- 出版方 / 主办方：USENIX Association
- CCF 官方目录入口：https://www.ccf.org.cn/Academic_Evaluation/By_category/

## 一句话定位

USENIX Security 是 CCF 推荐目录中 **网络与信息安全** 领域的 A 类会议。网络与信息安全方向的顶级 venue，适合密码学、系统安全、网络安全、隐私、安全攻防和安全协议研究。
...
- USENIX Security 官方定位为 security and privacy 顶级会议，重视系统、攻击、防御、测量和实践影响。
- 适合系统安全、软件安全、web/mobile/cloud、usable security、privacy、network security、ML security、hardware security。

### 官方投稿与评审要求

- 论文应清晰说明 threat model、现实影响、实验设计、伦理/披露流程和防御或分析价值。
- USENIX 风格通常重视可运行系统、真实测量、开源工件和实证严谨性。

### 给智能体的硬约束

- USENIX Security fit 要求实际安全/隐私贡献清楚。
- 没有现实威胁、数据或系统证据的泛论文章 fit 弱。

## 智能体提示词片段

```text
目标 venue 是 USENIX Security Symposium (USENIX Security)，CCF-A，类型为会议，所属 CCF 专业领域为：网络与信息安全 / Network and Information Security。
...
- 如果审稿人认为这是相邻领域论文，你如何解释它适合 USENIX Security？
