...
- 简写：TMM
- 全名：IEEE Transactions on Multimedia
- 文件名：TMM_CCFA.md
- 类型：期刊
- 榜单归属：CCF-A
...
TMM 是 CCF 推荐目录中 **计算机图形学与多媒体** 领域的 A 类期刊。计算机图形学与多媒体方向的顶级 venue，适合图形、视觉化、多媒体、VR/AR、渲染和交互媒体研究。
...
- 论文贡献与 IEEE Transactions on Multimedia 的全名和领域传统一致。
...
- TMM 官方定位为 multimedia 期刊，覆盖多媒体信号、内容、系统和应用。
- 适合 audio/image/video/text multimodal learning、retrieval、compression、streaming、multimedia systems、QoE、generation、understanding。

#### 官方投稿与评审要求

- 论文应体现多媒体或跨模态研究贡献，实验需覆盖合理任务、指标、数据和强 baseline。
- 系统类论文需讨论传输、交互、延迟、可扩展性或用户体验。

#### 给智能体的硬约束

- TMM fit 要求 multimedia/multimodal 核心。
- 单一模态普通分类若无多媒体问题，fit 弱。
...
```text
目标 venue 是 IEEE Transactions on Multimedia (TMM)，CCF-A，类型为期刊，所属 CCF 专业领域为：计算机图形学与多媒体 / Computer Graphics and Multimedia。
