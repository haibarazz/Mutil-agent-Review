...
- 简写：SC
- 全名：International Conference for High Performance Computing, Networking, Storage, and Analysis
- 文件名：SC_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
SC 是 CCF 推荐目录中 **计算机体系结构/并行与分布计算/存储系统** 领域的 A 类会议。计算机体系结构、并行与分布计算、存储系统方向的顶级 venue，适合硬件/系统/存储/HPC/云基础设施等底层计算系统研究。
...
- 论文贡献与 International Conference for High Performance Computing, Networking, Storage, and Analysis 的全名和领域传统一致。
...
- SC 官方定位为 high performance computing, networking, storage, and analysis 顶级会议。
- 适合 HPC architectures、systems、networks、storage、algorithms、applications、performance analysis、scientific computing、AI/HPC convergence。

#### 官方投稿与评审要求

- 论文通常需展示大规模计算、真实科学/工程应用、系统性能、可扩展性和可复现评估。
- SC 强调超算/高性能计算生态中的技术深度和实际影响。

#### 给智能体的硬约束

- SC fit 必须连接 HPC、large-scale systems、scientific workloads 或性能分析。
- 普通云服务或小规模 ML 实验如果没有 HPC 语境，fit 弱。
...
```text
目标 venue 是 International Conference for High Performance Computing, Networking, Storage, and Analysis (SC)，CCF-A，类型为会议，所属 CCF 专业领域为：计算机体系结构/并行与分布计算/存储系统 / Computer Architecture / Parallel and Distributed Computing / Storage System。
