...
- 简写：IEEE VIS
- 全名：IEEE Visualization Conference
- 文件名：IEEE_VIS_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
IEEE VIS 是 CCF 推荐目录中 **计算机图形学与多媒体** 领域的 A 类会议。计算机图形学与多媒体方向的顶级 venue，适合图形、视觉化、多媒体、VR/AR、渲染和交互媒体研究。
...
- 论文贡献与 IEEE Visualization Conference 的全名和领域传统一致。
...
- IEEE VIS 2026: https://ieeevis.org/year/2026/welcome

来源类型：官方 CFP / 官方会议页面

### 官方定位与主题范围

- IEEE VIS 官方定位为 visualization 旗舰会议。
- 适合 information visualization、scientific visualization、visual analytics、visualization systems、evaluation/user studies、perception/cognition、data storytelling。

### 官方投稿与评审要求

- 论文应提出可视化设计、系统、理论、技术或评估，并说明数据、任务、用户和视觉编码之间的关系。
- 评审重视 problem abstraction、design rationale、evaluation rigor 和 visualization insight。

### 给智能体的硬约束

- VIS fit 要求 visualization 是研究贡献，而不是只是画图展示结果。
- 缺少可视化任务/设计/用户或感知评估时 fit 弱。

## 智能体提示词片段

```text
目标 venue 是 IEEE Visualization Conference (IEEE VIS)，CCF-A，类型为会议，所属 CCF 专业领域为：计算机图形学与多媒体 / Computer Graphics and Multimedia。
...
- 如果审稿人认为这是相邻领域论文，你如何解释它适合 IEEE VIS？
