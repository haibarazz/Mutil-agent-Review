...
- 简写：NSDI
- 全名：Symposium on Network System Design and Implementation
- 文件名：NSDI_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
NSDI 是 CCF 推荐目录中 **计算机网络** 领域的 A 类会议。计算机网络方向的顶级 venue，适合网络协议、网络系统、移动/无线网络、互联网测量和网络体系结构研究。
...
- 研究问题属于 **计算机网络** 的核心问题。
- 论文贡献与 Symposium on Network System Design and Implementation 的全名和领域传统一致。
...
- NSDI 官方定位为 networked systems design and implementation。
- 适合网络化系统、分布式系统、云/边缘、数据中心、网络协议、系统安全、可靠性、存储/计算协同和生产系统经验。

### 官方投稿与评审要求

- 论文应有真实系统设计与实现，强调 design、implementation、deployment、measurement 和 lessons learned。
- 评审通常看系统完整性、实际部署价值、可扩展性和对社区的可迁移 insight。

### 给智能体的硬约束

- NSDI fit 必须是 networked systems，不是纯算法或纯网络理论。
- 缺少可运行系统或可信实现评估时 fit 弱。

## 智能体提示词片段

```text
目标 venue 是 Symposium on Network System Design and Implementation (NSDI)，CCF-A，类型为会议，所属 CCF 专业领域为：计算机网络 / Computer Networks。
