...
- 简写：CCS
- 全名：ACM Conference on Computer and Communications Security
- 文件名：CCS_CCFA.md
- 类型：会议
- 榜单归属：CCF-A
...
CCS 是 CCF 推荐目录中 **网络与信息安全** 领域的 A 类会议。网络与信息安全方向的顶级 venue，适合密码学、系统安全、网络安全、隐私、安全攻防和安全协议研究。
...
- 论文贡献与 ACM Conference on Computer and Communications Security 的全名和领域传统一致。
...
- CCS 官方定位为 computer and communications security 顶级会议。
- 适合系统安全、网络安全、密码协议应用、隐私、Web/mobile/cloud security、software security、ML security、usable security、measurement。

#### 官方投稿与评审要求

- 论文应明确 threat model、security goals、attack/defense novelty、实验或形式化验证，并充分讨论限制和伦理。
- 评审关注安全影响、技术正确性、可复现性、负责任披露和实际威胁相关性。

#### 给智能体的硬约束

- CCS fit 要求安全/隐私问题是核心贡献。
- 只有把安全作为应用场景、没有 threat model 或 security evaluation 时 fit 弱。
...
```text
目标 venue 是 ACM Conference on Computer and Communications Security (CCS)，CCF-A，类型为会议，所属 CCF 专业领域为：网络与信息安全 / Network and Information Security。
