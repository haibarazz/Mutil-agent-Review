...
- 简写：TPDS
- 全名：IEEE Transactions on Parallel and Distributed Systems
- 文件名：TPDS_CCFA.md
- 类型：期刊
- 榜单归属：CCF-A
...
TPDS 是 CCF 推荐目录中 **计算机体系结构/并行与分布计算/存储系统** 领域的 A 类期刊。计算机体系结构、并行与分布计算、存储系统方向的顶级 venue，适合硬件/系统/存储/HPC/云基础设施等底层计算系统研究。
...
- 论文贡献与 IEEE Transactions on Parallel and Distributed Systems 的全名和领域传统一致。
...
- TPDS 官方定位为 parallel and distributed systems 期刊。
- 适合并行算法/系统、分布式计算、云/边缘、HPC、调度、资源管理、容错、并行编程、分布式数据处理。

#### 官方投稿与评审要求

- 论文应证明并行/分布式方法在 scalability、performance、reliability 或 resource efficiency 上的贡献。
- 需要严谨实验、理论分析或大规模系统验证。

#### 给智能体的硬约束

- TPDS fit 要求并行或分布式机制本身是贡献。
- 只是用集群跑实验或部署应用，不等于 TPDS fit。
...
```text
目标 venue 是 IEEE Transactions on Parallel and Distributed Systems (TPDS)，CCF-A，类型为期刊，所属 CCF 专业领域为：计算机体系结构/并行与分布计算/存储系统 / Computer Architecture / Parallel and Distributed Computing / Storage System。
