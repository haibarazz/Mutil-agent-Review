...
- 简写：TC
- 全名：IEEE Transactions on Computers
- 文件名：TC_CCFA.md
- 类型：期刊
- 榜单归属：CCF-A
...
TC 是 CCF 推荐目录中 **计算机体系结构/并行与分布计算/存储系统** 领域的 A 类期刊。计算机体系结构、并行与分布计算、存储系统方向的顶级 venue，适合硬件/系统/存储/HPC/云基础设施等底层计算系统研究。
...
- 论文贡献与 IEEE Transactions on Computers 的全名和领域传统一致。
...
- IEEE Transactions on Computers: https://www.computer.org/csdl/journal/tc

来源类型：官方 Aims & Scope / About / Author 页面

### 官方定位与主题范围

- TC 官方定位覆盖计算机系统、体系结构、硬件/软件、可靠性、并行与分布式计算等 computing 主题。
- 适合计算机设计、体系结构、嵌入式/并行系统、可靠性、安全、性能评估和计算系统方法。

### 官方投稿与评审要求

- 稿件应是完整原创研究，强调技术深度、严谨实验/理论、清晰问题定义和对 computing 社区的贡献。
- 期刊稿件需要比会议版更充分的验证、比较和扩展。

### 给智能体的硬约束

- TC fit 的核心是 computer systems/computing；普通应用算法需说明其计算系统贡献。
- 缺少系统/架构/计算机制时 fit 弱。

## 智能体提示词片段

```text
目标 venue 是 IEEE Transactions on Computers (TC)，CCF-A，类型为期刊，所属 CCF 专业领域为：计算机体系结构/并行与分布计算/存储系统 / Computer Architecture / Parallel and Distributed Computing / Storage System。
