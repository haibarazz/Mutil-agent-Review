...
- 简写：TDSC
- 全名：IEEE Transactions on Dependable and Secure Computing
- 文件名：TDSC_CCFA.md
- 类型：期刊
- 榜单归属：CCF-A
...
TDSC 是 CCF 推荐目录中 **网络与信息安全** 领域的 A 类期刊。网络与信息安全方向的顶级 venue，适合密码学、系统安全、网络安全、隐私、安全攻防和安全协议研究。
...
- 论文贡献与 IEEE Transactions on Dependable and Secure Computing 的全名和领域传统一致。
...
- TDSC 官方定位为 dependable and secure computing，覆盖可靠性、安全性、隐私和可信计算。
- 适合容错、可靠性、可用性、安全机制、隐私保护、入侵检测、可信系统、风险评估和安全工程。

### 官方投稿与评审要求

- 论文应提供可验证的 dependable/security contribution，包含模型、系统、实验、形式化或实证证据。
- 期刊稿件需要完整讨论威胁/故障模型、假设、可扩展性和局限。

### 给智能体的硬约束

- TDSC fit 要求可靠性或安全性是中心。
- 普通系统优化没有 dependability/security 目标时 fit 弱。

## 智能体提示词片段

```text
目标 venue 是 IEEE Transactions on Dependable and Secure Computing (TDSC)，CCF-A，类型为期刊，所属 CCF 专业领域为：网络与信息安全 / Network and Information Security。
